...
| Power | `thermald` has no Wildcat Lake (`0xD5`) entry, exits at boot with `status=0/SUCCESS` so the failure is silent | **Fixed locally 2026-08-07** (one-line model-table entry, built from master, installed via a systemd drop-in to `/usr/local/bin`). Unlike `intel_idle` above, this table carries no measured constants (the entry is `adaptive_only`, so the platform's GDDV firmware tables supply every number), hence safe to patch without silicon measurement and worth sending upstream. Effect is **performance, not cooling**: sustained PL1 ceiling 15W → 25W. Load-tested 2026-08-07 and the throttle-down half of the loop never fires: GDDV binds the passive trip to `SEN1`, a platform thermistor that saturates ~26°C below the die (50°C vs 76°C package under 6-core load), so CPU work alone cannot reach the 54°C trip. Practical effect is the power uplift alone. Not filed upstream yet | [power/thermald-no-wildcat-lake-entry.md](power/thermald-no-wildcat-lake-entry.md) |
