...
| Power | `intel_idle` has no Wildcat Lake (`0xD5`) entry, falls back to ACPI `_CST` (no C1E, C10 advertised more conservatively) | Open, reported to linux-pm 2026-08-05. Not the S0ix blocker — C10 is entered normally. `CONFIG_INTEL_IDLE=y` so there's no DKMS route; a no-rebuild test via `intel_idle.table=` exists but was deliberately not run (low reward, can't add C1E) | [power/intel-idle-no-wildcat-lake-entry.md](power/intel-idle-no-wildcat-lake-entry.md) |
