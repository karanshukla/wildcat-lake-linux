...
|---|---|---|---|
| Display | PSR2/DSB deadlock causing glitches under GPU load | Worked around (kernel boot args), narrowed 2026-08-05 from `xe.enable_psr=0` to `xe.enable_psr=1` (PSR1 kept, PSR2 selective fetch still off) because disabling PSR entirely was what enabled LOBF; verification pending next reboot | [display/psr-dsb-deadlock.md](display/psr-dsb-deadlock.md) |
